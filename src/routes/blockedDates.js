...
router.get('/', authMiddleware, async (_req, res) => {
  try {
    ...
    res.json({
      dates,
      protected: config.protectedBlockedDates,
    });
  } catch (err) {
...
router.post('/', authMiddleware, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const dates = await getBlockedDateIds();
    res.status(201).json({ ok: true, dates, protected: config.protectedBlockedDates });
  } catch (err) {
    if (err.code === '23505') {
...
router.delete('/:date', authMiddleware, requireAdmin, async (req, res) => {
  ...
  if (config.protectedBlockedDates.includes(date)) {
    return res.status(403).json({ error: 'This date is permanently blocked.' });
  }

  try {
    await pool.query('DELETE FROM blocked_dates WHERE date = $1', [date]);
    const dates = await getBlockedDateIds();
    res.json({ ok: true, dates, protected: config.protectedBlockedDates });
  } catch (err) {
